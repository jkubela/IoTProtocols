...
print('********************************************************')
print('This is the test environment of the server')
print('********************************************************')
print('')
...
while payloadList:
	...
	while plrList:
		...
		reload(throughput)
		print('Starting Throughput test for AMQP with PLR : ' + str(plr) + ' and payload ' + str(payload_size))
		results = throughput.main(msg_payload, plr)

		with open('throughput_amqp.csv','a+') as csvfile:
			writer = csv.writer(csvfile, quoting=csv.QUOTE_ALL)
			writer.writerow(('round', 'msg_payload', 'plr', 'time_before_sending', 'time_after_sending', 'time_received'))
			writer.writerows([(data.round, data.msg_payload, data.plr, data.time_before_sending, data.time_after_sending, data.time_received) for data in results])
	
		with open('througput_amqp_'+ str(plr) + '_' + str(payload_size) + '.csv', 'a+') as singlefile:
			writer = csv.writer(singlefile, quoting=csv.QUOTE_ALL)
                        writer.writerow(('round', 'msg_payload', 'plr', 'time_before_sending', 'time_after_sending', 'time_received'))
                        writer.writerows([(data.round, data.msg_payload, data.plr, data.time_before_sending, data.time_after_sending, data.time_received) for data in results])
